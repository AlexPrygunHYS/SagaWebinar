...
namespace HotelBookingService
{
    internal class BookHotelHandler : 
        IHandleMessages<BookHotelRequest>,
        IHandleMessages<CancelHotelRequest>
    {
        ...
        {
            var result = new BookingHotelResponse { TripId = message.TripId};
        ...
        public async Task Handle(CancelHotelRequest message, IMessageHandlerContext context)
        {
            Log.Info($"Hotel for Trip Id {message.TripId} is canceled");
            var result = new BookingHotelResponse { TripId = message.TripId, IsBooked = false};
            await context.Publish(result);
        }
    }
}
